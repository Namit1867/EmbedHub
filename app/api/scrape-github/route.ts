...
export async function POST(request) {
  try {
    const { selectedRepos, token, owner, repo, branch } = await request.json();
    ...
    for (const repoObj of selectedRepos) {
      ...
      const { pathFromUrl } = parseRepoUrl(repoUrl);

      // Fetch the SHA and tree structure of the repository
      const sha = await fetchRepoSha(owner, repo, branch, pathFromUrl, token);
      ...
      const fileContents = await fetchFileContents(
        selectedFiles.map((file) => ({
          path: file.path,
          url: `https://api.github.com/repos/${owner}/${repo}/contents/${file.path}?ref=${branch}`,
        })),
        token
      );
